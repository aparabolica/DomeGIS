...
module.exports.before = function(hook) {
  return new Promise(function(resolve, reject) {
    ...
    if (!hook.data.query) reject(new errors.BadRequest("Missing SQL query."));
    else resolve();
  });
};
...
function generateDerivedTable(hook) {
  return new Promise(function(resolve, reject) {
    ...
    var createTableQuery =
      'SELECT * INTO "' + hook.data.id + '" from (' + sql + ") as derived";
    ...
    sequelize
      ...
      .then(function(queryResult) {
        ...
        sequelize.queryInterface
          ...
          .catch(function(err) {
            return reject(
              new errors.GeneralError("Error getting layer description.")
            );
          });
      })
      .catch(function(err) {
...
function updateExtentsIndexAndPermissions(hook) {
  return new Promise(function(resolve, reject) {
    ...
    var query =
      ...
      // 'ALTER TABLE "' +
      // layerId +
      // '" ADD COLUMN "domegis_id" SERIAL PRIMARY KEY;' +
      'GRANT SELECT ON "' +
      layerId +
      '" TO domegis_readonly;';
