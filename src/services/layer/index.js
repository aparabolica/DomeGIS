...
module.exports = function() {
  ...
  app.use("/layers/preview", function(req, res, next) {
    ...
    if (!sql) return res.status(400).json({ message: "Missing SQL query" });
    else
      sequelize_readonly
        .query(sql)
        .then(function(queryResult) {
          ...
          if (records.length == 0)
            return res
              .status(400)
              .json({ message: "Query returned no records." });
          else if (fields.length == 0)
            return res
              .status(400)
              .json({ message: "Query returned no fields." });
          else {
  ...
  app.use("/layers/:id/feature/:featureId", function(req, res, next) {
    ...
    Layers.get(layerId).then(function(layer) {
      ...
      var _from =
        'FROM "' +
        layerId +
        '" as t WHERE t.domegis_id = ' +
        parseInt(featureId);

      var select = "SELECT * " + _from;
      var extentSelect =
        "SELECT ST_Extent(ST_Transform(geometry,4326)) " + _from;
  ...
  app.use("/layers/:id/search", function(req, res, next) {
    ...
    Layers.get(layerId)
      .then(function(layer) {
        ...
        if (where.length > 0) {
          var query =
            'SELECT * FROM "' + layerId + '" as t WHERE ' + where.join(" OR ");
          sequelize
            .query(query)
            .then(function(queryResult) {
            ...
            .catch(function(err) {
              res
                .status(500)
                .json({ error: "error while searching in features" });
            });
    ...
    Layers.get(layerId)
      .then(function(layer) {
        ...
        if (!layer.featureCount)
          return res.status(500).json({ message: "Layer has no features." });

        var fields = [];
        async.eachSeries(
          layer.fields,
          function(field, doneEach) {
